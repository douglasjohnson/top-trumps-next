...
jest.mock('../service/DeckService');

const mockedFindAll = jest.mocked(findAll);
...
describe('Deck Admin', () => {
  ...
  it('should load decks', () => {
    mockedFindAll.mockImplementation(() => new Promise(jest.fn()));
  ...
  describe('edit', () => {
    ...
    beforeEach(async () => {
      ...
      mockedUpdate.mockImplementation(() => new Promise(jest.fn()));
      render(
        <QueryClientProvider client={queryClient}>
          <Suspense>
            <DeckAdmin />
  ...
  describe('create', () => {
    const mockedSave = jest.mocked(save);
    beforeEach(async () => {
      const decks: PersistedDeck[] = [{ id: '1', name: 'Deck 1', imageUrl: '', attributes: [], cards: [] }];
      mockedFindAll.mockResolvedValueOnce(decks);
      mockedSave.mockImplementation(() => new Promise(jest.fn()));
      render(
        <QueryClientProvider client={queryClient}>
          <Suspense>
            <DeckAdmin />
  ...
  describe('delete', () => {
    const mockedDelete = jest.mocked(deleteDeck);
    let deck: PersistedDeck;
    beforeEach(async () => {
      mockedDelete.mockImplementation(() => new Promise(jest.fn()));
      deck = { id: '1', name: 'Deck 1', imageUrl: '', attributes: [], cards: [] };
